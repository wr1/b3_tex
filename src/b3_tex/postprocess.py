...
class LoadcaseSolverSession(Protocol):
    ...
    def gp_coords(
        self,
    ) -> NDArray[np.float64]: ...  # (n_gp_total, 3) physical-space GP positions
...
def attach_homogenization_fields(
    ...
    *,
    ...
    c_diag_cell = (w[..., None] * c_diag.reshape(n_elem, nq, 6)).sum(axis=1) / cell_w[
        :, None
    ]
    ...
    if verbose:
        print(
            f"    phase 2: {len(loadcases)} stress-controlled back-solves", flush=True
        )
    eye6 = np.eye(6)
    points = grid.points
    ...
    for tag, k, desc in loadcases:
        ...
        residual = np.linalg.norm(r.macro_stress - sigma_target) / np.linalg.norm(
            sigma_target
        )
        ...
        sigma_cell = (w[..., None] * r.sigma_per_gp.reshape(n_elem, nq, 6)).sum(
            axis=1
        ) / cell_w[:, None]
        eps_cell = (w[..., None] * r.eps_per_gp.reshape(n_elem, nq, 6)).sum(
            axis=1
        ) / cell_w[:, None]
...
def material_sampling_uniformity(
    ...
    *,
            ...
            per_yarn.append(
                {
                    ...
                    "inplane_position": float(
                        getattr(yarn, "inplane_position", float("nan"))
                    ),
                    "n_gps": int(mask.sum()),
    ...
    bin_yarn_count = np.bincount(
        flat, weights=(ids_at_gps != 0).astype(float), minlength=n_total
    )
    ...
    spatial = {
        ...
        "cv_count": float(bin_count.std() / bin_count.mean())
        if bin_count.mean() > 0
        else 0.0,
        "matrix_only_min_count": (
            int(bin_count[bin_is_matrix_only].min())
            if bin_is_matrix_only.any()
            else None
        ),
    }
    ...
    return {
        ...
        "vf_ref": vf_ref,
        "vf_rel_diff": {
            n: abs(vf_fe[n] - vf_ref[n]) / max(vf_ref[n], 1e-30) for n in names
        },
        "per_yarn": per_yarn,
...
def _print_sampling_uniformity(report: dict) -> None:
    ...
    print(
        f"        n_cells = {report['n_cells']}, n_gp_total = {report['n_gp_total']}",
        flush=True,
    )
    print("      Vf recovery (FE GPs vs field-reference random sampling):", flush=True)
    for name in report["vf_fe"]:
        fe = report["vf_fe"][name]
        ref = report["vf_ref"][name]
        rel = report["vf_rel_diff"][name]
        print(
            f"        {name:>10s}:  FE = {fe:6.4f}  ref = {ref:6.4f}  "
            f"rel diff = {rel:.2e}",
            flush=True,
        )
    if report["per_yarn"]:
        print(
            "      per-yarn coverage (weighted volume of GPs inside each yarn body):",
            flush=True,
        )
        for y in report["per_yarn"]:
            print(
                f"        yarn {y['index']:>2d}  axis={y['axis']}  "
        ...
        print(
            f"        yarn coverage CV = {cv:.2e}  [{flag}]   "
            f"(< 5% expected for symmetric weaves)",
            flush=True,
        )
    sp = report["spatial"]
    cv_flag = "OK" if sp["n_empty"] == 0 else f"{sp['n_empty']} EMPTY BINS"
    print(
        f"      spatial GP-count uniformity ({sp['n_bins']} boxes): "
    ...
    if sp["matrix_only_min_count"] is not None:
        print(
            f"        ({sp['n_matrix_only']} matrix-only bins; "
...
def _print_eng_constants_crosscheck(
    ...
    print(
        "    cross-check engineering constants (algebraic vs from FE response):",
        flush=True,
    )
    print(
        f"        {'name':>6}  {'from S_eff':>14}  {'from loadcase':>14}  "
        f"{'rel diff':>10}",
        flush=True,
    )
    for k in _ENG_KEYS:
